...
def main():
    ...
    for loop in range(0,NvmBlockNumberAll):
        ...
        if tmpstr.find('[0]') == -1:
            IdDefineString += '''#define  '''+(((str(str(blockDict['RamAddress'])[1:]).replace('.', '_').replace('[', '_').replace(']', '_')).strip('(').strip(')')))+'''NvMBlockID    '''+str(loop+1)+'''U\n'''
        else:
			if tmpstr.find('[0]/**/') == -1:
				IdDefineString += '''#define  '''+(((str(str(blockDict['RamAddress'])[1:-3]).replace('.', '_')).strip('(').strip(')')))+'''NvMBlockID    '''+str(loop+1)+'''U\n'''
			else:
				IdDefineString += '''#define  '''+(((str(str(blockDict['RamAddress'])[1:-4]).replace('.', '_').replace('[', '_').replace(']', '_')).strip('(').strip(')')))+'''NvMBlockID    '''+str(loop+1)+'''U\n'''
		#print blockDict
        #写ncm_cfg
        #NvFileHnd.write(
