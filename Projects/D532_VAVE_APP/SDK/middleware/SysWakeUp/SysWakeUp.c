...
boolean gSystemsleeping = FALSE;
boolean gInterruptAwake = FALSE;
...
static void SysAccAndLviDetWakeUpISR(void)
{
	...
	{	
		...
		//INT_SYS_DisableIRQ(PORTE_IRQn);//not close it's interrupt , closed at after wake up .
	#endif

	if(FALSE == LPTMR_DRV_IsRunning(INST_LPTMR1))
	{
		LptmrStartCounter();
	}
}
