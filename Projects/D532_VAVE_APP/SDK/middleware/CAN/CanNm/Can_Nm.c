...
void CanNm_EcuRxIndication( uint8_t canInstance ,PduIdType RxPduId, const PduInfoType* PduInfoPtr)
{
	...
	//0X35D
	if(RxPduId == IPDU_HND_BCM_SystemControl)
	{
		//masterWakeUpSleepCmd
		SleepM_RxIndication(0,(PduInfoPtr->SduDataPtr[1]&0X03));
	}
}
