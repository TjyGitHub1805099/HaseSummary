...
extern const flexcan_user_config_t *Can_InitConfig[];
extern flexcan_state_t *Can_cfg_State[];
extern const uint8 CanIf_RxFilterNum[];
extern const CanIf_RxIdFilterType *CanIf_RxIdFilter[];
extern flexcan_msgbuff_t *CanIf_RecvMsgBuff[];
extern const uint16 CanTxStartIndex[CAN_INSTANCE_COUNT];
extern const uint16 CanIfNumOfTxPdu[CAN_INSTANCE_COUNT];
extern const uint16 CanIfNumOfRxPdu[CAN_INSTANCE_COUNT];
extern const CanIf_TxPduCfgType  CanIf_TxPduCfg[CANIF_TX_PDU_NUM];
extern const CanIf_RxPduCfgType * const CanIf_RxPduCfg[CAN_INSTANCE_COUNT];
...
extern const void (*CanIf_BusOffCbk)(uint8_t instance);

extern uint32 gDiagRandomNumber;
extern volatile uint8 gSleepM_WakeUp_Flag2;
...
void CanIf_Init(uint8 Instance)
{
	...
	if(FLEXCAN_DRV_Init(Instance, Can_cfg_State[Instance], Can_InitConfig[Instance]) != STATUS_SUCCESS)
	{
    ...
    case FLEXCAN_EVENT_RX_COMPLETE:
        ...
        if (isMatch) {
            ...
            if (CanIfUserRxIndications[mdType] != NULL) {
				...
				CanNm_EcuRxIndication(0,pduId, &pduInfo);
				//used in can nm:end
				...
				if( TRUE != gSleepM_WakeUp_Flag2 )
				{
					...
					{
						//if((0X00 != (pduInfo.SduDataPtr[0]&0X01)) || (0X00 != (pduInfo.SduDataPtr[5]&0X0F)) || (0X00 != (pduInfo.SduDataPtr[6]&0X80)))
						if(0X00 != (pduInfo.SduDataPtr[2]&0X01))
						{
							gSleepM_WakeUp_Flag2 = TRUE ;
						}
					}
