...
extern const uint16 Com_IpduMaxNum;
extern const Com_SignalIdType Com_IpduId2SigId[][2];
extern uint8 * const Com_IpduBufPtr[];
extern Type1FuncPtrType const Com_IpduInformCbk[];
extern Type1FuncPtrType const Com_IpduErrorCbk[];

extern const uint16 Com_RxTempQueueSize;
extern const uint16 Com_RxIpduMaxNum;
extern volatile uint32 * const Com_RxIpduQueueMgrPtr[];
extern uint8 * const Com_RxIpduQueuePtr[];
extern volatile uint32 Com_RxTemp_TickBuf[];
extern volatile uint32 Com_RxIpdu_TickBuf[];
extern const Com_RxIpduConfType Com_RxIpduConfInfo[];
extern volatile uint8 Com_RxTempQueue[][8];
extern PduIdType Com_RxTempQueueMgr[];

extern const uint16 Com_TxIpduMaxNum;
extern const uint16 Com_TxIpduStartIndex;
extern const Com_TxIpduConfType Com_TxIpduConfInfo[];
...
extern PduIdType *Com_IpduGroupPtr[];
#endif
...
static void Com_Inner_RxProcess(void)
{
    ...
    uint32 l_sysTick = 0u;
    uint32 l_pduLen = 0u;
    uint16 l_rxIpduLoop = CQUEUE_ERR_PUSHPOP, l_RxSigLoop = 0u, l_queueIndex = CQUEUE_ERR_PUSHPOP;
...
void Com_Init(void)
{
...
#endif
    Com_IpduBufChangeQueueCh = CQueue_Init(CQUEUE_BUFTYPE_UINT16, Com_RxBufChangeQueue, Com_RxIpduMaxNum);
    //Com_RxTempQueueCh = CQueue_Init(CQUEUE_BUFTYPE_UINT16, Com_RxTempQueueMgr, Com_RxTempQueueSize);
    Com_TxTrigQueueCh = CQueue_Init(CQUEUE_BUFTYPE_UINT16, Com_TxTrigQueue, Com_TxIpduMaxNum);
    if ((CQUEUE_ERR_CHANNEL != Com_IpduBufChangeQueueCh)
        ...
        for (l_rxPduId = 0; l_rxPduId < Com_RxIpduMaxNum; l_rxPduId++) {
            if (0 != Com_RxIpduConfInfo[l_rxPduId].QueueDepth) {
                Com_RxIpduQueueCh[l_rxPduId] = CQueue_Init(CQUEUE_BUFTYPE_UINT32, Com_RxIpduQueueMgrPtr[l_rxPduId], Com_RxIpduConfInfo[l_rxPduId].QueueDepth);
...
{
	uint16 i = 0;
	uint8 reqType = CanAppTxInitValue;
