...
boolean Flash_Sector_Programing(uint32 addr, uint8 *data, uint16 wr_size )
{
     ...
     * Flash Command Complete event */
	  ...
	  if(ret != STATUS_SUCCESS)
	  	return ret;
	  //DEV_ASSERT2(STATUS_SUCCESS == ret,2);
	  ...
	  if(ret != STATUS_SUCCESS)
	  	return ret;
	  //DEV_ASSERT2(STATUS_SUCCESS == ret,3);
	
	  /* Verify the program operation at margin level value of 1, user margin */
	  ret = FLASH_DRV_ProgramCheck(&pSSDConfig, address, size, data, &failAddr, 1u);
	  if(ret != STATUS_SUCCESS)
	  	return ret;
	  //DEV_ASSERT2(STATUS_SUCCESS == ret,4);
