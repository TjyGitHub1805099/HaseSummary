...
uint16 CQueue_Init(uint16 channel, CQueue_BufType bufType, void *buffer, uint16 size)
{
    uint16 l_chLoop = 0u;
    ...
    if ((NULL != buffer) && (0 != size) && (size < 0XFFFF)) {
        if ((channel < CQUEUE_CHANNEL_MAX)
            ...
            CQueue_InitCfg[channel].bufType = bufType;
            CQueue_InitCfg[channel].size = size;
            CQueue_InitCfg[channel].bufPtr = buffer;
            CQueue_Manager[channel].head = 0;
            CQueue_Manager[channel].tail = 0;
            CQueue_Manager[channel].count = 0;
#if(SYS_OS_NOOS != SYS_OS_TYPE)
            if (E_OK == CQueue_Mutex_Create(channel)) {
                CQueue_Manager[channel].state = CQUEUE_STATE_INIT;
                l_return = channel;
            } else {
                CQueue_Manager[channel].state = 0;
            }
#else
            CQueue_Manager[channel].state = CQUEUE_STATE_INIT;
            l_return = channel;
        ...
        } else if (channel == CQUEUE_CH_AUTO) {
            for (l_chLoop = 0; l_chLoop < CQUEUE_CHANNEL_MAX; l_chLoop++) {
                if (0 == (CQueue_Manager[l_chLoop].state & CQUEUE_STATE_INIT)) {
                    CQueue_EnterCriticalCode();
                    CQueue_InitCfg[l_chLoop].bufType = bufType;
                    CQueue_InitCfg[l_chLoop].size = size;
                    CQueue_InitCfg[l_chLoop].bufPtr = buffer;
                    CQueue_Manager[l_chLoop].head = 0;
                    CQueue_Manager[l_chLoop].tail = 0;
                    CQueue_Manager[l_chLoop].count = 0;
#if(SYS_OS_NOOS != SYS_OS_TYPE)
                    if (E_OK == CQueue_Mutex_Create(l_chLoop)) {
                        CQueue_Manager[l_chLoop].state = CQUEUE_STATE_INIT;
                        l_return = l_chLoop;
                    } else {
                        CQueue_Manager[l_chLoop].state = 0;
                    }
#else
                    CQueue_Manager[l_chLoop].state = CQUEUE_STATE_INIT;
                    l_return = l_chLoop;
