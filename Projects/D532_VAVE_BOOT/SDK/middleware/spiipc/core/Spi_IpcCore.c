...
if(SpiCrcSupport)\
{\
	(pack_len) += 4;		/* add crc word length */ \
	(ch) |= IPC_CRC_MASK;\
}\
...
sint32 SetupAndProcessMasterWriteExt(uint8 HwChannel,uint8 *pBuff)
{
	...
	//memcpy(&TempSpiReadBuf[HwChannel][0], &TempSpiReadBuf[HwChannel][2], ReadLen);
	{
		Ch = TempSpiReadBuf[HwChannel][2]&IPC_CHANNEL_MASK;

		if(1 == HwChannel)	//now used for da channel
		{
			Spi_MComFLag = TRUE;
		}
		...
		else if (IPC_FLOWCTRL_CHANNEL != Ch)
		{
			...
			while(1)
			{
				...
				pos += 1;
				err = SPI_CQueue_Push(HwChannel, SPI_CQueueGetRxManagerHandle(HwChannel), Ch, &TempSpiReadBuf[HwChannel][1+pos], TempSpiReadBuf[HwChannel][pos]);	//length remove channel byte
				if(err != E_OK)
					break;
...
uint8_t PrepareWriteMessae(uint8 HwChannel)
{
	...
	if(qlen > 0)
	{
		...
		else
		{
			...
			//TempSpiWriteBuf[HwChannel][2] = ch&IPC_CHANNEL_MASK;
...
void HandleSPI_IPC_Messages(uint8 HwChannel)
{
	...
	if((err == E_OK) && (GetCurSpiStatus(HwChannel) == SPI_STATUS_SUCESS))
	{
		...
		else 	
		{
			...
			vTaskDelay(1U); //Wait a minute for recovery
		}
	}
	else
	{
...
sint32 SetupAndProcessMasterWrite(uint8 HwChannel,uint8 *pBuff)
{
	...
	//Setup the dma for the rx data - blocking read
	...
	if(err == E_OK)
	{
		...
		if (IPC_FLOWCTRL_CHANNEL != Ch)
		{
			...
			if(MultiFlag == 0)
			{
				SPI_CQueue_Push(HwChannel, SPI_CQueueGetRxManagerHandle(HwChannel), Ch, &TempSpiReadBuf[HwChannel][1], ReadLen-1);	//length remove channel byte
				spiSemPost(HwChannel);
			}
			else if(HwChannel == 0)
			{
