...
tUdsSessionInnerType sid0x10GetCurSession(uint32 ch)
{
	return gUdsSessionType[ch];
}



tUdsNRCType sid0x10SetSession(uint32 ch,tUdsSessionInnerType newSessionType)
{
	uint8 i = 0;
	tUdsNRCType ret = NRC_NONE;
	uint8 supportedSidTotalCount = udsSidGetSupportedSidCount(ch);
	...
	case UDS_DEFAULT_SESSION:		
		...
		{
			if(gDiagSidService[ch][i].stopHookFunc != NULL)
			{
				gDiagSidService[ch][i].stopHookFunc(ch);
			}
		}
		gUdsSessionType[ch] = UDS_DEFAULT_SESSION;
		break;
	default:
		//default to None-default
		if(gUdsSessionType[ch] == UDS_DEFAULT_SESSION)
		{
			gUdsSessionType[ch] = newSessionType;
		}
		else // none-default to none-default
		{
			ret = sid0x10ChangeSessionStrategy(ch,newSessionType);
			if(ret == NRC_NONE || ret == NRC_RESPONSE_PENDING)
			{
				for(i=0;i<supportedSidTotalCount;i++)
				{
					...
					sid0x27ResetStatus(ch);
					#endif
					if((gDiagSidService[ch][i].sessionTypeFilter & newSessionType) == 0)
					{
						if(gDiagSidService[ch][i].stopHookFunc != NULL)
						{
							gDiagSidService[ch][i].stopHookFunc(ch);
