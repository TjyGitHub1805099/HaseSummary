...
boolean sid0x27SetSecurityStatus(tUdsSecurityStatus securityStatus)
{
	...
	case UDS_SECURITY_STATUS_LOCKED:
		...
		diagNvmWriteSync(gSid0x27CfgInfor.nvmBlockIdForAuthFailCounter,&gKeyAuthFailCounter);
		break;
...
void sid0x27Init()
{
	...
	if(gSid0x27CfgInfor.relockByFailCounterIfSysReset == TRUE)
	{
		...
		if(gSid0x27CfgInfor.nvmBlockIdForAuthFailCounter != 0)
			diagSetNvmBlockId(gSid0x27CfgInfor.nvmBlockIdForAuthFailCounter,TRUE);
	...
	{
		if(gSid0x27CfgInfor.nvmBlockIdForLockTimer != 0)
