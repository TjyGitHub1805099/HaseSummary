	...
	if(gForwardReqToDiagAppFlag == FALSE)
	{
		...
		if(didList!=NULL)
		{
			...
			else
			{
				...
				{
					diagForwardReqToApplayer(gUdsCfgInfor.diagAppTaskId,ch,data,toSocDidNumList,toSocDidCount*2+2);
...
*/
tUdsNRCType diagSid0x2fIOControl(uint8 ch,  uint8 *data,uint16 len)
{
	...
	if(gForwardReqToDiagAppFlag == FALSE)
	{
		...
		ret = sid0x2fMainProcess(ch,data,len);
		if(ret == NRC_RESPONSE_PENDING) // we will get resp async
		{
			gForwardReqToDiagAppFlag = TRUE;
			gGetRespFromDiagAppFlag = FALSE;
...
tUdsNRCType diagSid0x31RoutineControl(uint8 ch,  uint8 *data,uint16 len)
{
	...
	if(gForwardReqToDiagAppFlag == FALSE)
	{
		...
		ret = sid0x31MainProcess(ch,data,len);
...
#ifdef UDS_WITH_DTC
tUdsNRCType diagSid0x14ClearDTC(uint8 ch,  uint8 *data,uint16 len)
{
	tUdsNRCType ret = NRC_SUB_FUNCTION_NOT_SUPPORTED;
	...
	ret = dtcCoreClearDtc(data+1,len-1);
	if(ret == NRC_NONE)
		gRespDataLen = 0;
#endif
	return ret;
...
tUdsNRCType diagSid0x85DTCSetting(uint8 ch,  uint8 *data,uint16 len)
{
	uint8 dtcSettingType = 0;
	tUdsNRCType ret = NRC_SUB_FUNCTION_NOT_SUPPORTED;

	if(data == NULL || len !=2)
		return NRC_INCORRECT_MESSAGE_LENGTH;

	ret = sid0x85CheckPreCondition(data,len);
	if(ret != NRC_NONE)
		return ret;

	dtcSettingType = data[1] & 0x7F;
	switch(dtcSettingType)
	{
	case DTC_SETTING_STATUS_ON:
	case DTC_SETTING_STATUS_OFF:
		sid0x85SetDTCSettingStatus(dtcSettingType);
		break;
	default:
		return NRC_SUB_FUNCTION_NOT_SUPPORTED;
	}
...
tUdsNRCType diagSid0x19ReadDtc(uint8 ch,  uint8 *data,uint16 len)
{
	tUdsNRCType ret = NRC_SUB_FUNCTION_NOT_SUPPORTED;
	...
	{	
		...
		if((ret=sid0x19CheckPreCondition(data,len)) != NRC_NONE)
			return ret;

	#if 0
		diagForwardReqToApplayer(gUdsCfgInfor.dtcCoreTaskId,ch,data,data,len);
	#else
		gForwardReqToDiagAppFlag = TRUE;
		gUDSBufferPtr = data+1;
		diagSaveChId(ch);
		if((ret=dtcCoreReadDtcInfor(data,len))!=NRC_NONE)
		{
			gForwardReqToDiagAppFlag = FALSE;
			return ret;
		}
