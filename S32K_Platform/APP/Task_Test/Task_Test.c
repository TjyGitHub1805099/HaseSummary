...
#include "Task_Test_Cfg.h"
#if (STD_ON == TASK_TEST_DEBUG_TRACE)
#include "trace.h"
#endif
/*********** START: INCLUDE FILES ************/
...
void Task_Test(void * pvParameters)
{
...
#if (STD_ON == TASK_TEST_STACK_MONITOR)
	...
	System_Signal_Subscribe(SYSTEM_SIG_ID_10MS, SYSTEM_TASK_ID_TEST);
	//System_Signal_SubSpecValue(SYSTEM_SIG_ID_XXX, SYSTEM_TASK_ID_TEST, X);
	...
	while (1)
	{
		l_returnValue = System_Task_ReceiveMessage(SYSTEM_TASK_ID_TEST, &l_msgInfo, FALSE, OS_WAITFOREVER);
		if (E_OK == l_returnValue)
		{
			...
			{
#if (STD_ON == TASK_TEST_DEBUG_TRACE)
				TracePrintf(TEST_TRACE, TRACE_INFO, "INFOR:[Task_Test-I]\r\n");
				TracePrintf(TEST_TRACE, TRACE_ERRO, "ERR:[Task_Test-I]\r\n");
#endif
			}
...
#if (STD_ON == TASK_TEST_DEBUG_TRACE)
			TracePrintf(TEST_TRACE, TRACE_WARNING, "[Task_Test-W]:Stack Size:%d\r\n",l_stackSize);
#endif
#endif
		}
	}
}
/*=========================== END OF FILE: Task_Test.c ===========================*/
