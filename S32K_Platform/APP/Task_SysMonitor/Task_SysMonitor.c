...
void Task_SysMonitor(void * pvParameters)
{
	...
	System_Signal_Subscribe(SYSTEM_SIG_ID_100MS, SYSTEM_TASK_ID_SYSMONITOR);
	...
	while (1)
	{
		...
		if (E_OK == l_returnValue)
		{
			...
			{
			#if (STD_ON == TASK_SYSMONITOR_DEBUG_TRACE)
				TracePrintf(SYSTEM_TRACE, TRACE_INFO, "[Task_SysMonitor-I]:fead WDG:%d\r\n",System_GetTickValue());
			#endif
...
#if (STD_ON == TASK_SYSMONITOR_DEBUG_TRACE)
			TracePrintf(SYSTEM_TRACE, TRACE_WARNING, "[Task_SysMonitor-W]:Stack Size:%d\r\n",l_stackSize);
#endif
#endif
		}
	}
