...
#=====================================================================文件查找注释开始
def findVaribleRelation(fileDir,fileName,l_var):
    #clear back_file
    l_fileName = fileDir + "\\" + fileName.replace(".","_back.")
    if(os.path.isfile(l_fileName)):
        fp_back = open(l_fileName, 'w')
        fp_back.truncate()#clear file
        print("clear",l_fileName)
    else:
        fp_back = open(l_fileName, mode="w", encoding="utf-8")
        print("create",l_fileName)
...
def main():
    ...
    findVaribleRelation(CUR_PY_DIR,'function.map','gSleepM_WakeUp_Flag')
    findVaribleRelation(CUR_PY_DIR,'main.c','gSleepM_WakeUp_Flag')
