...
uint8 gAvmCanMsgState[AVM_CAN_MSG_STATE_LEN] = {0,0,0,0};	
...
extern uint32 System_GetTickValue();
extern sint32 api_amp_get_diag_result(uint8* result);
...
void IVI_dtcAppReportCanCommuFault(boolean timeoutFlag, uint16 ecuId,uint8 canMsgMask,boolean semiFlag)
{
	...
	else
	{
		if(timeoutFlag==FALSE)
			gIviCanMsgSemiFlag[ecuId] |= canMsgMask;
		
		if(gIviCanMsgSemiFlag[ecuId] & canMsgMask)
		{
			if(timeoutFlag == TRUE)
				gIVICanFaultRealtimeFlag[ecuId] |= canMsgMask;
			else
...
void IC_dtcAppReportCanCommuFault(boolean timeoutFlag, uint16 ecuId,uint8 canMsgMask,boolean semiFlag)
{
	...
	{
		if(timeoutFlag==FALSE)
			gIcCanMsgSemiFlag[ecuId] |= canMsgMask;
		
		if(gIcCanMsgSemiFlag[ecuId] & canMsgMask)
		{
			if(timeoutFlag == TRUE)
				gIcCanFaultRealtimeFlag[ecuId] |= canMsgMask;
			else
...
void AVM_dtcAppReportCanCommuFault(boolean timeoutFlag, uint16 ecuId,uint8 canMsgMask,boolean semiFlag)
{
	...
	{
		if(timeoutFlag==FALSE)
			gAvmCanMsgSemiFlag[ecuId] |= canMsgMask;
		
		if(gAvmCanMsgSemiFlag[ecuId] & canMsgMask)
		{
			if(timeoutFlag == TRUE)
				gAvmCanFaultRealtimeFlag[ecuId] |= canMsgMask;
			else
...
void dtcCheckVoltage()
{
	...
	if(sysVolStatus == DIAG_SYS_VOLTAGE_STATUS_HIGH)
	{
	...
	if(sysVolStatus == DIAG_SYS_VOLTAGE_STATUS_NORMAL)
	{
...
void dtcApp_SetVehicleSpeedTimeoutFlag(boolean flag)
{	
...
void dtcAppReportAvmMsgToSoc(uint8 ignStatus,uint32 curSysTick)
{
	...
	avmMsgStatus[0] = gDtcAppCanFaultBitRealtimeArray[UDS_AVM][AVM_FAULT_INDEX_RX_CAN_MSG_BSW]&AVM_RX_CAN_MSG_MASK_BSW_0X57A ? 0:1;	
...
void dtcAppReportAvmMsgToSoc(uint8 ignStatus,uint32 curSysTick)
{
	...
	avmMsgStatus[0] = gDtcAppCanFaultBitRealtimeArray[UDS_IVI][IVI_FAULT_INDEX_RX_CAN_MSG_BSW]&IVI_RX_CAN_MSG_MASK_BSW_0X57A ? 0:1;	
	...
	for(i=0;i<2;i++)
	{
		...
		if(avmMsgStateReverseFlag[i] == TRUE)
		{
			if(avmMsgStatus[i] == 0)
			{
				if(curSysTick - avmMsgStableStatusCounter[i] >= 1700)
				{
					avmMsgStatusCfmFlag[i] = TRUE;
					gAvmCanMsgState[2*i-1] = TRUE;
...
void dtcAppInit()
{
	...
	gDtcAppCanFaultTotalNum[UDS_IVI] = IVI_FAULT_INDEX_MAX;	
	gDtcAppCanFaultTotalNum[UDS_IC] = IC_FAULT_INDEX_MAX;	
	gDtcAppCanFaultTotalNum[UDS_AVM] = AVM_FAULT_INDEX_MAX;	
	...
	for(ch=0;ch<UDS_CH_MAX_NUM;ch++)
	{
		for(i=0; i<gDtcAppCanFaultTotalNum[ch]; i++)
		{
			...
			if(i== DTC_APP_FAULT_INDEX_CAN_BUS_OF)
			{
				...
				gDtcAppCanFaultBitArray[ch][i] &= 0xff7f; 
			...
			else
			{
				...
				{
					gDtcAppCanFaultBitRealtimeArray[ch][i] = 0;
					gDtcAppCanFaultBitArray[ch][i] = 0x00; 
					gDtcAppCanFaultInforPtr[ch][i].faultActive = FALSE;
					gDtcAppCanFaultInforPtr[ch][i].JT1_timerActiveFlag = FALSE;
				}
				else
			...
			if((gDtcAppCanFaultBitArray[ch][i]&DTC_APP_THIS_CYCLE_TEST_FAILED) == 0)
			{
				if((gDtcAppCanFaultInforPtr[ch][i].opt_cond_level & ECU_OPT_COND_LEVEL_3) == 0)
				{
...
void dtcAppLoop()
{
			...
			for(ch=0;ch<UDS_CH_MAX_NUM;ch++)
			{
				for(k=DTC_APP_RX_CAN_MSG_FAULT_START_INDEX; k<gDtcAppCanFaultTotalNum[ch]; k++)
				{
						...
						|| (ch==UDS_AVM && k == AVM_FAULT_INDEX_RX_CAN_MSG_HCM)))
					{
						...
						gDtcAppCanFaultBitArray[ch][k] |= 0x80;
	...
	Alarm_GetStatus(SYSTEM_SIG_ID_5S, &SleepStatus); 
		...
		for(i=DTC_APP_RX_CAN_MSG_FAULT_START_INDEX; i<gDtcAppCanFaultTotalNum[ch]; i++)
		{
			...
			if(gDtcAppCanFaultInforPtr[ch][i].faultActive !=  curFaultStatus)
			{
				...
				else
				{
					...
					gDtcAppCanFaultBitArray[ch][i] &= 0x7F3F;
					gDtcAppCanFaultInforPtr[ch][i].preTick = diagGetCurTick();
					gDtcAppCanFaultInforPtr[ch][i].JT1_timerActiveFlag = FALSE;
				}
			...
			if(gDtcAppCanFaultInforPtr[ch][i].faultActive == TRUE)
			{
					...
					&& gDtcAppCanFaultInforPtr[ch][i].JT2_timeoutFlag == FALSE)
				{
					if((curTick - gDtcAppCanFaultInforPtr[ch][i].preTick) >= gDtcAppCanFaultInforPtr[ch][i].JT2_timerValue)
					{
						...
						gDtcAppCanFaultBitArray[ch][i] &= 0xFFC0;	
						gDtcAppCanFaultBitArray[ch][i] |= VALMAX_VALUE;
	...
	{		
		...
		l_u16Value = 0;
		Com_ReceiveSignal(SIG_HND_VSO, &l_u16Value);
		...
		if(diagMuxOnMsgRxTimeoutFlag == FALSE)
		...
		l_u16Value = 0;
		Com_ReceiveSignal(SIG_HND_TWN, &l_u16Value);
		if(l_u16Value != 0xFF)
	...
	AdcWrapper_Read(ADC_LOGIC_CH_GPS_ANT_PWR, &l_u16Value);      
