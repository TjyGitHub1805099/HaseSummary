...
{
	return (t0x22DidInfo*)sid0x22GetDidInfo(devCh, did);
}
...
{
	return (tIOCtrlInfo*)sid0x2fGetIoCtrlInfo(ch, did);
}
...
tUdsNRCType ameReadData(uint32 devCh, uint8* data, uint16 len)
{
	...
	if(didList != NULL)
	{
		...
		toSocDidNumListWithSid[0] = SID_READ_DATA_BY_DID;    
		...
		{	
			...
			if(ameDidInfo != NULL)
			{
				if(ameDidInfo->fromSocFlag == TRUE)
				{
					...
					diagCpyData(&toSocDidNumListWithSid[(toSocDidCount-1)*2+2], (const uint8*)(&data[i*2+1]), 2);
		...
		{			
			...
			ipc_gw_send_request(0, IPC_CHANNEL_DIAG, IVI_IPC_AME_READ, toSocDidNumListWithSid, toSocDidCount*2+2, &ctx, (invoke_callback)ameIpcCbk_McuRequestSocReply);
...
tUdsNRCType ameWriteData(uint32 devCh, uint8* dataInfo, uint16 len)
{
	...
	{	
		...
		if(didInfor->fromSocFlag == TRUE)
		{
			...
			ctx.sequence_number = 0;
			ipc_gw_send_request(0, IPC_CHANNEL_DIAG, IVI_IPC_AME_WRITE, &dataInfo[0], len, &ctx, (invoke_callback)ameIpcCbk_McuRequestSocReply);
		...
		else if(didInfor->nvmAddr != NULL)
		{
			...
			{
				diagCpyData(didInfor->nvmAddr, (const uint8*)didValue, didValueLen);
...
void ameIpcCbk_McuRequestSocReply(request_context_t* ctx, uint16 function, uint8* rx_data, uint32 len)
{
	...
	ameAppFillTxBuffer(&rx_data[1], len-1);      

	devCh = ameParseDevChannel(function);
	ameAppTxData(devCh, ameTxBufferPtr->buffer, ameTxBufferPtr->curDataLen);     
