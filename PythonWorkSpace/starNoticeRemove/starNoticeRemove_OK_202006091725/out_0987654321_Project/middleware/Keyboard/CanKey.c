...
static uint16 ReverseBit(uint16 data)
{
	...
	l_retVal = (l_retVal << 8) | (l_retVal >> 8);               
	l_retVal = ((l_retVal << 4)&0xf0f0)|((l_retVal>>4)&0x0f0f); 
	l_retVal = ((l_retVal << 2)&0xcccc)|((l_retVal>>2)&0x3333); 
	l_retVal = ((l_retVal << 1)&0xaaaa)|((l_retVal>>1)&0x5555); 
	return l_retVal;
}
#endif
