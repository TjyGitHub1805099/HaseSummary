...
#define	TP_WRAPPER_TIMER_VALUE_N_AR (gTpCfgInfor.timerValuesCfg->timerValue_N_AR) 
#define TP_WRAPPER_TIMER_VALUE_N_BR (gTpCfgInfor.timerValuesCfg->timerValue_N_BR) 
#define TP_WRAPPER_TIMER_VALUE_N_CR (gTpCfgInfor.timerValuesCfg->timerValue_N_CR) 
		
#define	TP_WRAPPER_TIMER_VALUE_N_AS (gTpCfgInfor.timerValuesCfg->timerValue_N_AS) 
#define TP_WRAPPER_TIMER_VALUE_N_BS (gTpCfgInfor.timerValuesCfg->timerValue_N_BS) 
#define TP_WRAPPER_TIMER_VALUE_N_CS (gTpCfgInfor.timerValuesCfg->timerValue_N_CS) 
...
void Tp_RxIndication(PduIdType RxPduId, const PduInfoType* data)
{
	uint16 dataLen = 0;
	...
	if(dataLen>TP_BUF_LEN || dataLen <= 7)
	{
		return;
	}
	...
	for(i=0;i<TP_PDU_GROUP_NUM;i++)
	{
		...
		{	
			...
			#else
			if(circleListInsert(&gTpRxCirlceListInfor[i], (uint8*)PduInfoPtr->SduDataPtr,dataLen) == FALSE)
				gTpRxErrCounter++;
			#endif
			#endif
...
boolean Tp_TxDataWrapper(uint8 ch,const uint8* data, uint8 len)
{
	...
	if(!data || len != TP_BUF_LEN)
		return FALSE;

	pduData.SduDataPtr = (SduDataPtrType)data;
	pduData.SduLength = 8;
...
boolean Tp_TxDataRequest(uint32 ch, uint8*data, uint16 len)
{
	...
	if(ch >= TP_PDU_GROUP_NUM || data==NULL || len == 0)
		ret = FALSE;
	else if(TP_PDU_ID_FOR_TX_ADDR(ch) != 0) 
	{
		if(TRUE == TX_NEW_FRAME_FLAG(ch))
			ret = FALSE;
		else
		{
...
void Tp_TxConfirmation(PduIdType TxPduId)
{
	uint8 i = 0;
...
void Tp_CheckTxcfm(uint8 i)
{
	...
	if(gTxcfmCh[i]!= TP_PDU_GROUP_NUM)
	{
		...
		case TP_ACTION_SEND_FC:
			if(RX_FF_DL(ch) > 0) 
				Tp_SetTimer(ch,TP_TIMER_N_CR);
			else
				Tp_ResetParas(ch);
			break;
		case TP_ACTION_SEND_FF:
			RX_ALLOWED_FRAME_TYPE(ch) = TP_FRAME_TYPE_FC;
			Tp_SetTimer(ch,TP_TIMER_N_BS);
			break;
		case TP_ACTION_SEND_CF:
			if(NEXT_ACTION(ch) == TP_ACTION_IDLE)
			{
				if(TP_TRACE_LEVEL > TP_TRACE_LEVEL_2)
				{
...
boolean Tp_SetTimer(uint8 ch,tTpTimerType type)
{
	if(TP_TRACE_LEVEL > TP_TRACE_LEVEL_3)
	{
		TP_TRACE_VALUE2(TP_TRACE,TRACE_INFO,"TP:ch:%d setTimer:%d\n\r",ch,type);
	}
...
{
	TP_TRACE_VALUE2(TP_TRACE,TRACE_INFO,"TP:ch:%d,cancel timer",i,TP_TIMER_TYPE(i));
	TP_TIMER(i) = 0;
...
void Tp_Init()
{
	...
	if(gTpInitFinishedFlag == FALSE)
	{
		for(i=0;i<TP_PDU_GROUP_NUM;i++)
		{
			Tp_ResetParas(i);
			gTxcfmCh[i] = TP_PDU_GROUP_NUM;
			if(circleListInit(&gTpRxCirlceListInfor[i], gTpRxCirlceListInfor[i].itemBufferLen, gTpRxCirlceListInfor[i].listNum) == FALSE)
				return;
		}
...
void Tp_TimerProcess(uint8 ch)
{
	...
	default:
		{
			TP_TRACE_VALUE2(TP_TRACE,TRACE_ERRO,"TP: ch:%d Tp_TimerProcess(): invalid timer type[%d]\n\r",ch,TP_TIMER_TYPE(ch));
		}
	}

	if(TP_TRACE_LEVEL > TP_TRACE_LEVEL_2)
	{
		TP_TRACE_VALUE2(TP_TRACE,TRACE_INFO,"TP:ch:%d timeout:T:%d\n\r",ch,TP_TIMER_TYPE(ch));
	}
...
boolean Tp_ProcessFirstFrame(uint8 ch)
{
	...
	if(TP_TRACE_LEVEL > TP_TRACE_LEVEL_2)
	{
		TP_TRACE_VALUE2(TP_TRACE,TRACE_INFO,"TP: ch:%d rx FF: DL=%d\n\r",ch,reqBufferLen);
	}

	if(reqBufferLen < TP_BUF_LEN)
	{
		TP_TRACE_VALUE2(TP_TRACE,TRACE_WARNING,"TP: ch:%d rx invalid FF: DL=[%d]\n\r",ch,reqBufferLen);
		Tp_ResetParas(ch);
		return FALSE;
	}
	...
	if(NULL!=gUpperLayerWrapper[TP_DIAG_CH_TYPE(ch)].FnRxFirstFrameIndication)
	{
		...
		case BUFFER_OK:
			...
			if(TP_TRACE_LEVEL > TP_TRACE_LEVEL_2)
			{
				TP_TRACE_VALUE1(TP_TRACE,TRACE_INFO,"TP: ch:% tx FC\n\r",ch);
			}
...
void Tp_ProcessConsecutiveFrame(uint8 i)
{
	...
	if(TP_TRACE_LEVEL > TP_TRACE_LEVEL_2)
	{		
		TP_TRACE_VALUE4(TP_TRACE,TRACE_INFO,"TP:ch:%d rxCF:Seq:0x%x,0x%x,%d\n\r",i,RX_CF_SEQ(i),RX_EPT_CF_SEQ(i),RX_BS_COUNTER(i));
	}
	...
	if(RX_EPT_CF_SEQ(i) == RX_CF_SEQ(i))
	{
		...
		if(RX_CF_COUNTER(i) < RX_CF_SUM(i))
		{
			if(TP_WRAPPER_BS == 0 || RX_BS_COUNTER(i) < TP_WRAPPER_BS)
			{
				Tp_SetTimer(i,TP_TIMER_N_CR);
			}
			else if(RX_BS_COUNTER(i) >= TP_WRAPPER_BS)
			{
				if(TP_TRACE_LEVEL > TP_TRACE_LEVEL_2)
				{
					TP_TRACE_VALUE1(TP_TRACE,TRACE_INFO,"TP: ch:%d,Tp_PCF,resp FC\r\n",i);
				}
		...
		else 
		{
			if(TP_TRACE_LEVEL > TP_TRACE_LEVEL_2)
			{
				TP_TRACE_VALUE1(TP_TRACE,TRACE_INFO,"TP: ch:%d Got all CF\r\n",i);
			}
			ff_len = RX_FF_DL(i);
			Tp_ResetParas(i);
	...
	else
	{
		if(TP_TRACE_LEVEL > TP_TRACE_LEVEL_2)
		{
			TP_TRACE_VALUE3(TP_TRACE,TRACE_INFO,"TP:ch:%d,Err:SEQ is wrong,eptSeq:0x%x,curSeq:0x%x\r\n",i,RX_EPT_CF_SEQ(i),RX_CF_SEQ(i));
		}
...
boolean Tp_ProcessFlowControlFrame(uint8 i)
{
	...
	{
		TP_TRACE_VALUE1(TP_TRACE,TRACE_INFO,"TP: ch:%d,Rx FC\n\r",i);
	}
	...
	case TP_FS_CTS:
		...
		RX_FC_BS(i) = blocksize;
		RX_FC_ST_MIN(i) = STmin>0x7F? 0x7F : STmin;
		
		TX_BS(i) = blocksize;
		TX_STMIN(i) = STmin>0x7F? 0x7F : STmin;;
		
		Tp_EnableSTminTimer(i);
...
void Tp_RxProcess(uint8 i)
{
	...
	if(gTpRxBuffer[i].len>0 && gTpRxBuffer[i].len<=TP_BUF_LEN)
	{
		...
		gTpRxBuffer[i].len = 0;
	#else
	while(isCircleListEmpty(&gTpRxCirlceListInfor[i]) == FALSE)
	{
		if(circleListPop(&gTpRxCirlceListInfor[i], localBuffer, TP_BUF_LEN) == FALSE)
		{
			circleListClear(&gTpRxCirlceListInfor[i]);
			return;
		}
	#endif
		ret = Tp_ParseFrame(i,localBuffer, TP_BUF_LEN);
		if(ret == TRUE)
		{
			Tp_Memset((uint8 *)tpRxBuffer[i],TP_BUF_LEN,0);

			if(Tp_CpyData((uint8 *)tpRxBuffer[i],(const uint8 *)localBuffer,TP_BUF_LEN) == TRUE)
			{
				...
				if(RX_ALLOWED_FRAME_TYPE(i) & RX_CUR_FRAME_TYPE(i))
				{
					...
					case TP_FRAME_TYPE_SF:
						...
						if(TP_TRACE_LEVEL > TP_TRACE_LEVEL_3)
						{
							TP_TRACE_VALUE2(TP_TRACE,TRACE_INFO,"TP:ch:%d FnRxIndication,data[0]=0x%x\n\r",i,tpRxBuffer[i][1]);
						}
						if(NULL!=gUpperLayerWrapper[TP_DIAG_CH_TYPE(i)].FnRxIndication)
							gUpperLayerWrapper[TP_DIAG_CH_TYPE(i)].FnRxIndication(i,(uint8 *)&tpRxBuffer[i][1],tpRxBuffer[i][0]);
...
void Tp_TxProcess(uint8 i)
{
	...
	if(NEXT_ACTION(i) == TP_ACTION_SEND_CF)
	{
		...
		if(gTimerSTmin[i].timeoutFlag == TRUE)
		{

			if(TP_TRACE_LEVEL > TP_TRACE_LEVEL_2)
			{
				TP_TRACE_VALUE2(TP_TRACE,TRACE_INFO,"TP:ch:%d Tx CF,seq:0x%x\n\r",i,TX_SEQ(i));
			}
			
			gTimerSTmin[i].timeoutFlag = FALSE;
			...
			if(gUpperTxDataInfor[i].restDataLen > TP_CONSECUTIVE_FRAME_DATA_LEN) 
			{
			...
			else
			{
				...
				if(TP_TRACE_LEVEL > TP_TRACE_LEVEL_2)
				{
					TP_TRACE_VALUE2(TP_TRACE,TRACE_INFO,"TP:ch:%d Tx all CF,seq:0x%x\n\r",i,TX_SEQ(i)-1);
				}
	...
	else if((NEXT_ACTION(i) == TP_ACTION_SEND_SF) || (NEXT_ACTION(i) == TP_ACTION_SEND_FF))
	{
		#if 0
		...
		if(gUpperTxDataInfor[i].dataLen > TP_CONSECUTIVE_FRAME_DATA_LEN)
		{
			if(TP_TRACE_LEVEL > TP_TRACE_LEVEL_2)
			{
				TP_TRACE_VALUE1(TP_TRACE,TRACE_INFO,"TP:ch:%d Tx FF\n\r",i);
			}
		...
		else
		{
			if(TP_TRACE_LEVEL > TP_TRACE_LEVEL_2)
			{
				TP_TRACE_VALUE1(TP_TRACE,TRACE_INFO,"TP:ch:%d Tx SF\n\r",i);
			}
