...
#include "trace.h"
//uint8 g_cUartPanelCQueueCh = 0xFF;
//uint8 g_cUartPanelCQueueBuffer[UART_PANEL_CQUEUE_BUFF_LEN];
uint8 g_cPanelRxRawDataBuffer[PANEL_RX_RAW_DATA_BUFF_LEN];
uint8 g_cUartPanelDeviceInitDone = FALSE;
uint16 g_sSecondPanelSendDataFlag = 0xffff;  //Data Updata Bit Flag

//SystemMutexType semUartPanelMutex;
...
//	static boolean firstInterruptFlag = TRUE;
//	static uint8 filterCounter = 0;

//	if(firstInterruptFlag == TRUE)
//	{
...
//			if((gUartRxFrameLen + rxCount) <UART_IPC_RX_BUFF_LEN)
//			{
//				memcpy(gUartRxDataBuffer+gUartRxFrameLen,gUartDriverRawDataBuffer,rxCount);
//				gUartRxFrameLen += rxCount;
//			}
//			else
//				;//exceed buffer,data lost
			rxCount = 0;
		}

//		if(event == UART_EVENT_END_TRANSFER)
//		{
//			if(gUartRxFrameLen>0)
//			{
//				CQueue_Push(uartIpcCQueueCh,gUartRxDataBuffer,gUartRxFrameLen,STD_ON);
//				gUartRxFrameLen = 0;
//			}
//			memset(gUartRxDataBuffer,0,UART_IPC_RX_BUFF_LEN);
...
//		#endif
		...
		{
//			System_Mutex_Create(semUartPanelMutex);
			ret = TRUE;
//			if((g_cUartPanelCQueueCh = CQueue_Init(CQUEUE_BUFTYPE_UINT8, g_cUartPanelCQueueBuffer, UART_PANEL_CQUEUE_BUFF_LEN)) != CQUEUE_ERR_CHANNEL)
//			{
...
uint8 GetSecondPanelSoundWare(uint8 cData)
{
	...
	{
		//cSoundWraeSignal = stSecondPanelPara.SoundWare + 1;
		//Com_TxSig_Lamp_Music_Sig(COM_SET_SIG_VALUE, &cSoundWraeSignal);	//Set CAN SoundWare Sig
	}
	return E_OK;
}
...
uint8 SendDataToSecondPanel(void)
{
	static uint8 i = 0;
	uint8 j = 0;
	static uint8 cCycleUpdateDataFlag = 0;  //Cycle SendData

	if(E_OK == GetFlagBit(&g_sSecondPanelSendDataFlag, SecondPanel_ValidEnable))  //OLED enable
	{
		switch (i)
		{
			case 0:
