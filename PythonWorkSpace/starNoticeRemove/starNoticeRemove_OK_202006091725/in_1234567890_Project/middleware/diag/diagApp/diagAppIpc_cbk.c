...
void diagApp_AVM_IpcRequestInd(const void *ctx, uint8 *rx_data, uint32 len)
{
	...
	case AVM_IPC_POWER_CTRL_REQ:
		if(rx_data[0] == 0)
		{
			Port_SetOutLevel(CAMMERA1_ON,PORT_LOW);//0--close,1--open camera power
			Port_SetOutLevel(CAMMERA2_ON,PORT_LOW);//0--close,1--open camera power
			Port_SetOutLevel(CAMMERA3_ON,PORT_LOW);//0--close,1--open camera power
			Port_SetOutLevel(CAMMERA4_ON,PORT_LOW);//0--close,1--open camera power
		}
		else
		{
			Port_SetOutLevel(CAMMERA1_ON,PORT_HIGH);//open camera
			Port_SetOutLevel(CAMMERA2_ON,PORT_HIGH);//open camera
			Port_SetOutLevel(CAMMERA3_ON,PORT_HIGH);//open camera
			Port_SetOutLevel(CAMMERA4_ON,PORT_HIGH);//open camera
		}
